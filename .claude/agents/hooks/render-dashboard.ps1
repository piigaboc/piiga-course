...
# Renders agents/dashboard/index.html from the DASHBOARD.md table.
# DASHBOARD.md stays the source of truth; this just produces a viewable HTML page.
# Shared hub copy: lives in agents/hooks and writes the one shared dashboard,
# so every linked project renders the same board.
$ErrorActionPreference = 'SilentlyContinue'

$md     = Join-Path $PSScriptRoot '..\DASHBOARD.md'
$outDir = Join-Path $PSScriptRoot '..\dashboard'
...
<head>
  ...
  <title>PiigaBoc Product Dev Agents -- Status Dashboard</title>
  <style>
    /* -- Design tokens ------------------------------------------- */
    :root {
      --bg-base:       #eef0f3;
    ...
    *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    /* accent bar (left stripe) - per-agent color set inline via style attribute */
    .card-accent {
      width: var(--accent-bar);
      flex-shrink: 0;
      background: var(--c-unused);
    }
    ...
    /* status-specific card treatments */
    .card.idle:hover {
      box-shadow: 0 6px 18px rgba(22,163,74,0.10), 0 2px 6px rgba(0,0,0,0.06);
    }
    .card.working {
    ...
    .card.working:hover {
      box-shadow: 0 0 0 1px rgba(217,119,6,0.30), 0 8px 22px rgba(217,119,6,0.12);
    }
    .card.blocked {
      border-color: var(--c-blocked-border);
      box-shadow: 0 0 0 1px rgba(220,38,38,0.12), 0 2px 8px rgba(220,38,38,0.06);
    }
    .card.blocked:hover {
      box-shadow: 0 0 0 1px rgba(220,38,38,0.24), 0 8px 22px rgba(220,38,38,0.08);
    }
    .card.unused {
      opacity: 0.65;
    }
  ...
  <header class="page-header">
    <div class="header-top">
      <div class="header-titles">
        <h1>PiigaBoc Product Dev <span>Agents</span> &mdash; Status Dashboard</h1>
      </div>
      <div class="project-chip">piiga group</div>
    </div>
    <div class="legend">
